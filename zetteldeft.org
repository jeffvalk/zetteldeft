...
*** New file creation
**** =zetteldeft-filename-separator= separates the ID from the title in filename creation

Filenames are constructed using the note's ID and title, separated by this string.

Note that file creation and renaming are handled by =deft=, which allows this separator value to be effected by =deft-file-naming-rules=, if specified.
...
**** =zetteldeft--lift-file-title= returns note title

Returns the title as parsed by =deft-parse-title= from the note's buffer contents and file path.
If =deft-use-filename-as-title= is set, this returns the file name, ignoring the buffer's contents.
...
The prompt for new note title provides the current note title (as returned by =zetteldeft--lift-file-title=) as the default value.

If the file does not have an existing zetteldeft ID, a new ID is generated. Hence, this function may be used on a file, which was not originally created with zetteldeft, in order to allow it to participate in zetteldeft's ID-based linking.
